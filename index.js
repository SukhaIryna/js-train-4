...
function checkIfIncludes(mainStr, subStr) {
  // Використовуємо метод , щоб перевірити, чи входить підрядок subStr в рядок mainStr
...
function padString(original, length, padWith, side) {
  ...
  if (side === "start") {
    let result = original.padStart(length, padWith);
    return result;
  }
  if (side === "end") {
    let result = original.padEnd(length, padWith);
    return result;
  } else {
...
function trimString(original, side) {
  ...
  if (side === "both") {
    ...
    return "Error: side should be either 'both', 'start' or 'end'";
  }
